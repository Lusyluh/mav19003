window.addEventListener('DOMContentLoaded',() =>{
    ...
    //variables that we will need for the game
    let board = ['','','','','','','','','']
    let currentPlayer = 'X' ;
    let isGameActive = true;

    const PLAYERX_WON = 'PLAYERX WON';
    const PLAYERO_WON = 'PLAYER0 WON';
    const TIE = 'TIE';
    /* Indexes within the playing board
    [0] [1] [2]
    [3] [4] [5]
    [6] [7] [8]
    */
...
if (!board.includes(''))
   ...
   //announce the winner and the score
   const announce = (type) =>{
       switch(type){
           case PLAYERO_WON:
   ...
   //clear the board for a replay by clicking the reset button
   const resetBoard = () => {
    board = ['', '', '', '', '', '', '', '', ''];
    isGameActive = true;
